...
A production 500 that took down every Ollama request on the shared Spark host, traced to a five-condition MoE + CUDA interaction inside Ollama itself — not a bug in this bot. Recorded here, unfixed, because every option is either server-side on a host this project doesn't own, or client-side and unmeasured.
...
## 6 — Options considered, none taken
...
| 2 | Lower `num_ctx` client-side | Free and entirely ours — we never exceed 3,544 of the 8,192 allocated. **But the matrix only proves 4096 safe, and 4096 is too small for us:** worst case is ~3,866 tokens (1,116-token system prompt + 6 chunks × 400 `chunk_max_tokens` + tool schemas + question), leaving ~230 tokens for an answer whose observed max is 617. Something between ~5,100 and the crash boundary would likely work but is **unmeasured** — see the time-sensitivity warning above. |
...
The only thing standing between this bot and a repeat of this incident is a server-side setting this project does not control. If flash attention on the host is ever turned off — for us, or for an unrelated reason — nothing stops another team from re-enabling it during a later Ollama or driver upgrade, with no notice to us. If that happens, the crash comes back and, without this document, it would look like a brand-new mystery instead of a five-condition failure mode we already understand.
