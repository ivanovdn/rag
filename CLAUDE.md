...
| `⚠️ Policy service temporarily unavailable` reaches users; log shows repeated `AgentWorkflowStartEvent` cycles ending in `[worker] Unavailable notice sent` | Not a bot bug — retry-then-notice is the resilience layer working as designed. Seen in production: a CUDA fault on the shared Ollama host (`an illegal memory access was encountered`, HTTP 500) poisons the model runner; every request fails until Ollama replaces it, then it self-heals. Confirm with `curl http://172.20.0.22:11434/api/ps`; a poisoned runner fails a short prompt and a long one alike, a healthy host serves both. |
