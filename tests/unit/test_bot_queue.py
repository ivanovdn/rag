...
# fake prints something, not that the real code does. To reach the transient LLM branch
# without a real network or LLM, only a dependency deep inside _run_rag is faked
# (rag.resilience.retry_transient) — same module-attribute-patch technique as
# tests/unit/test_resilience.py's `time.sleep` patch. Retrieval now runs before the agent
# (rag.search_first.prefetch), so it must resolve to "ok" here or these tests would reach
# a real, un-mocked search — prefetch's own unavailable-logging is covered by
# tests/unit/test_search_first.py::test_an_unavailable_prefetch_logs_which_component_failed
# instead, which is where retrieval failure now belongs.
...
def test_run_rag_logs_transient_llm_failure_type_and_message(monkeypatch, capsys):
    ...
    monkeypatch.setattr(sf, "prefetch", lambda q: sf.PrefetchResult("ok", "[Source 1] x"))
